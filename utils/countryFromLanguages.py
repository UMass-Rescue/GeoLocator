...
def get_location_from_language(language_code):
    ...
    try:
        ...
        if language:
            ...
            countries = [country.name for country in pycountry.countries if language.name in country.official_languages or language.name in getattr(country, 'languages', [])]
            print(countries,"countriess")
            if countries:
                print(countries)
                return countries
            else:
                return f"No specific country found for the language code '{language_code}'"
        else:
